...
@app.get("/books/get_by_rating/", status_code=status.HTTP_200_OK)
...
@app.get("/books/get_by_year/", status_code=status.HTTP_200_OK)
async def read_by_year(year: int = Query(gt=0)):
    books_to_return = [book for book in BOOKS if book.published_year == year]
    return books_to_return
