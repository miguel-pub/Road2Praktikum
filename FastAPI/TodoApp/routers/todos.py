...
async def read_todo(user: user_dependency,
    ...
    todo_model = db.query(Todos).filter(Todos.id == todo_id).filter(Todos.owner_id == user.get("id")).first() #first saves performance
...
async def update_todo(user: user_dependency,
                      db: db_dependency,
                      todo_request: TodoRequest,
                      todo_id: int = Path(gt=0)): # todo_reqeust needs to come before smth with Path
    if user is None:
        raise HTTPException(status_code=401, detail="Failed")   
